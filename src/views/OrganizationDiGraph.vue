<template>
  ...
  <div style="position: absolute; width: 100%; height: 100%">
    <div style="position: absolute; bottom: 5px; left: 5px; right: 5px">
      <el-input
        v-model="hasPermId2"
        placeholder="请输入要查询关系的两个企业的PermID, 以逗号分隔"
      >
...
export default {
  ...
  data() {
    return {
      hasPermId2: "4295864222,5000066483",
    };
  },
  computed: {
    hasPermIdA() {
    ...
    cypher() {
      return `match p=(o1:Organization{hasPermId:"${this.hasPermIdA}"})-[*2]-(o2:Organization{hasPermId:"${this.hasPermIdB}"}) RETURN p LIMIT 100`;
    },
  },
  async created() {
    if (this.hasPermId2) {
